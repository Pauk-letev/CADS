...
public class CoutEventArgs
{
    public string Message { get; }
    public CoutEventArgs(string message)
    {
        Message = message;
    }
}
delegate void Cout(object sender, CoutEventArgs e);
interface IName
{
    public string Name { get; set; }
    public event Cout? PrintName;
    public void Print();
}
class Subject : IName
{
    ...
    public void Print()
    {
        PrintName?.Invoke(this, new CoutEventArgs("Название предмета: " + Name + ", оценка: " + Mark));
    }
}
class Student : IInstitute, IName
{
    public string InstituteName { get; set; }
    public int Year { get; set; }
    public string Fucalty { get; set; }
    ...
    public void Print()
    {
        PrintName?.Invoke(this, new CoutEventArgs("Студент: " + Name + "\nИнститут: " + InstituteName + "\nКафедра: " + Fucalty +
             "\nГруппа: " + Group + "\nКурс: " + Year + "\n"));
        for (int i = 0; i < subject.Length; ++i)
            subject[i].Print();
        PrintName?.Invoke(this, new CoutEventArgs("\n"));
    }
...
class DataBase
{
    ...
    public void Remove(Student student, bool setting = false)
    {
        int found = -1;
        for (int i = 0; i < size; ++i)
            if (dataBase[i].Name == student.Name)
                found = i;
        if (found == -1)
            return;
    ...
    public void Edit(Student student)
    {
        ...
        switch (choice)
        {
            case 1:
                ...
                dataBase[found].Name = Console.ReadLine();
                break;
            case 2:
...
class Program
{
    public delegate string Responses(ref DataBase dataBase);
    ...
    {
        Console.WriteLine("Нажмите 1, чтобы вывести следующие запросы: \n");
        ...
        Console.WriteLine("Нажмите 2, чтобы поизучать ошибки\n");
        Console.WriteLine("Нажмите 0, чтобы изменить данные в базе.\n");
    }
    ...
    static string Response3(ref DataBase dataBase)
    {
        ...
        for (int i = 1; i < 6; ++i)
            if (dataBase.expelled[i] > dataBase.expelled[k])
                k = i;
        return "Больше всего было исключено студентов " + k + "-го курса";
    }
    ...
    static string Response5(ref DataBase dataBase)
    {
        ...
        foreach (Student student in dataBase.dataBase)
            if (student.Great())
                result += student.Name + ", " + student.InstituteName + ", " + student.Group + ", " + student.Year.ToString() + "-й курс\n";
        return result;
    }
    ...
    {
        string result = "Список отличников со второго курса:\n\n";
        foreach (Student student in dataBase.dataBase)
            if (student.Year == 2 && student.Great())
                result += student.Name + ", " + student.InstituteName + ", " + student.Group + "\n";
        return result;
    }
    ...
    private static void Student_PrintName(object sender, CoutEventArgs e)
    {
    ...
    static void Request(ref DataBase dataBase)
    {
        ...
        responses += Response15;
        responses += Response16;
        responses += Response17;
        responses += Response18;
        responses += Response19;
        responses += Response20;
        responses += Response21;
        responses += Response22;
        responses += Response23;
        while (true)
        {
            Instructions();
            req = int.Parse(Console.ReadLine());
            if (req == 1)
                foreach(Responses resp in responses.GetInvocationList())
                {
                    result += resp(ref dataBase) + "\n\n";
                }
            else if (req == 2)
            {
                Console.WriteLine("Выберете номер ошибки, на которую вы хотели бы посмотреть: ");
            ...
            else result = "Запрос не определен";
            Console.Clear();
            Output(result);
    ...
    static void Save(DataBase dataBase)
    {
        using (StreamWriter sw = new StreamWriter("base.txt", false))
        {
            foreach(Student student in dataBase.dataBase)
            {
                sw.WriteLine(student.Name + " " + student.InstituteName + " " + student.Fucalty + " " +
...
class Exceptions
{
    ...
    public static void Examples(int i)
    {
        switch (i)
        {
            case 1:
                ...
                {
                    Console.WriteLine("Переполнение стека вызовов: " + ex.Message);
                }
                break;
            case 2:
                try
                ...
                {
                    Console.WriteLine("Несоответствие типа типу массива: " + ex.Message);
                }
                break;
            case 3:
                try
            ...
            case 4:
                ...
                {
                    Console.WriteLine("Выход за граница массива: " + ex.Message);
                }
                break;
            case 5:
                try
                ...
                {
                    Console.WriteLine("Неверное приведение типов: " + ex.Message);
                }
                break;
            case 6:
                try
                ...
                {
                    Console.WriteLine("Выход за пределы памяти: " + ex.Message);
                }
                break;
            case 7:
                try
