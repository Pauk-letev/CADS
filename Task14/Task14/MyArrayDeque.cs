class MyArrayDeque<T>
{
    ...
    public void AddLast(T obj)
    {
        ...
        tail = Prev(head); elements[tail] = obj;
    }
    ...
    public bool RemoveFirstOccurrence(object obj)
    {
        ...
        int index = tail;
        while (!obj.Equals(elements[index]))
            index = Next(index);
        while (Prev(index) != tail)
        {
            elements[index] = elements[Prev(index)];
            index = Prev(index);
        }
        tail = Next(tail);
        return true;
    ...
    public bool RemoveLastOccurrence(object obj)
    {
        ...
        while (!obj.Equals(elements[index]))
            index = Prev(index);
        while (Next(index) != head)
        {
            elements[index] = elements[Next(index)];
            index = Next(index);
        }
        head = Prev(head);
        return true;
